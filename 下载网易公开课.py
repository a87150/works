...
save_path = "E:\BaiduYunDownload"#os.environ['HOMEPATH']
    ...
    def getVideo(self):
        for li in self.urlist:
            ...
            if url_match:
                ...
                if self.single:
                    ret = urllib.request.urlretrieve(mp4url, title)
                else:
                    ret = urllib.request.urlretrieve(mp4url, save_path+self.dirname+"/"+title)
...
if __name__ == "__main__":
    input_url = input("请输入一个网易公开课的链接(单集链接或者课程链接): ")
